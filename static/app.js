...
(() => {
  ...
  function renderRoute(data) {
    ...
    replace(routeResult, title, kinds, totals, rail, footer);
  ...
  async function submitRoute(e) {
    ...
    replace(routeResult, message('경로를 찾는 중이에요. 실시간 정보까지 확인하느라 몇 초 걸릴 수 있어요.'));
    const signal = begin('route');
    try {
      renderRoute(await api('/api/route', params, signal));
    } catch (err) {
